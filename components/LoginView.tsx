...
import { getGeminiApiKey, syncGeminiApiKeyFromAIStudio } from '../lib/getGeminiApiKey';
  ...
  useEffect(() => {
    const checkKey = async () => {
      ...
      if (window.aistudio?.hasSelectedApiKey) {
        ...
        if (hasKey) {
          await syncGeminiApiKeyFromAIStudio();
          setAutoStart(true);
        }
      }
    };
        ...
        <div className="bg-slate-900/50 backdrop-blur-3xl border border-white/5 p-10 rounded-[3rem] shadow-2xl space-y-8">
          ...
          <div className="space-y-4">
            ...
            >
              ...
              {autoStart ? 'START ENGINE' : 'SIGN IN WITH GOOGLE'}
                </>
              )}
            </button>
            <a 
